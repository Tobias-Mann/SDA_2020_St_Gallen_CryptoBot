...
# Calculate the moving average - MINUTES
...
plt.title('MOVING AVERAGES (MINUTES)')
plt.show()

# Calculate the moving average - HOURS
hourly['MA_40'] = hourly['Close'].rolling(window=40).mean()
hourly['MA_252'] = hourly['Close'].rolling(window=252).mean()
hourly[['Close', 'MA_40', 'MA_252']].plot(figsize=(10, 10))
plt.title('MOVING AVERAGES (HOURS)')
plt.show()

# Calculate the moving average - DAYS
daily['MA_40'] = daily['Close'].rolling(window=40).mean()
daily['MA_252'] = daily['Close'].rolling(window=252).mean()
daily[['Close', 'MA_40', 'MA_252']].plot(figsize=(10, 10))
plt.title('MOVING AVERAGES (DAYS)')
plt.show()
